...
- Cada afirmação em `mundo.md` sai marcada: `[fonte: <URL>]`, `[fonte: você]` (o que o usuário
  disse) ou `[não verificado]`.
- `[não verificado]` é permitido em `mundo.md` e **proibido** em `traducao.md`: mecânica sem
  fonte não vai à mesa.
- Não sabe? Pergunte. Perguntar custa uma linha; presumir custa uma campanha inteira montada
  no mundo errado.
...
Treze critérios. Cada um fica preenchido com fonte, ou `NÃO SE APLICA — <motivo>`. Nenhum fica
em branco.
...
| 3 | Tom e limites de conteúdo | Decide o que é apropriado mais do que qualquer regra |
| 4 | A lei do mundo | O que ali ocupa o lugar da magia: radiação, tecnologia, dobra, fé, tecnologia alienígena. Sem isso a magia de `docs/` entra sem fachada e o mundo some |
| 5 | Escala e geografia | Decide viagem (`docs/regras/exploracao.md`) e o tamanho do arco |
| 6 | Facções, três no mínimo, com o que cada uma quer | Motor de aventura mais barato que existe |
...
| 13 | Seis ganchos tirados das facções | É o que `planejar-aventura` consome direto |
...
puxe os critérios 2, 4, 6 e 11, que é onde a identidade mora.

Mundo capturado sobrevive à troca de nome. Mundo copiado, não.

## Fase 5 — A tradução

Cinco tabelas em `traducao.md`, cada linha citando o arquivo de `docs/`:
